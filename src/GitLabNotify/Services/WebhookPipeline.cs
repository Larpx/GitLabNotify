...
namespace Larpx.PersonalTools.GitLabNotify.Services
{
    ...
    /// 接收端写入 Channel 后立即返回，由后台服务异步消费并转发。
    ...
    public class WebhookPipeline : BackgroundService
    {
        ...
        public WebhookPipeline(
            ...
            ILogger<WebhookPipeline> logger)
        {
        ...
        protected override async Task ExecuteAsync(CancellationToken stoppingToken)
        {
            ...
            try
            {
                await foreach (var task in _channel.Reader.ReadAllAsync(stoppingToken))
                {
                    ...
                    {
                        // 兜底:Processor 自身已尽量吞异常,这里再次捕获并落库 Failed,
                        // 避免日志报错但数据库仍停留在旧状态。
                        ...
                        {
                            _logger.LogError(statusEx, "更新记录状态为 Failed 失败，记录 ID={RecordId}", task.RecordId);
                        }
                    }
                }
            }
